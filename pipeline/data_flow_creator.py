...
class DataFlowCreator:
    def __init__(self, images_path="./", batch_size=32, image_size=None, randomize=False, flipping=False, rotation=False, shearing=False, zooming=False,
    ...
    def feed_from_json_files(self, train_label_path, valid_label_path):
        try:
            with open(train_label_path, 'r') as train_data:
                self.train_set = json.loads(train_data)
        except IOError:
            print("Le fichier suivant n'a pas pu être ouvert, : " + train_label_path)

        try:
            with open(valid_label_path, 'r') as valid_data:
                self.train_set = json.loads(valid_data)
        except IOError:
    ...
    def get_train_set(self):
        try:
            ...
            return
        # TODO faire générator

    def get_train_set(self):
        for batch_data in self.train_set:
            yield self.load_image_set(batch_data)

    @staticmethod
    def load_image_set(self, batch_data):
        data = []
